...
from data_vessels import VesselsDataset_in3, VesselsDataset_in5
from transformations import (
    random_affine, random_hsv, random_vertical_flip, random_horizontal_flip,
    to_torch_tensors, pad_images_unet, random_cutout, random_crop
)
...
def crop_576(imgs):
    return random_crop(imgs, size=(576, 576))
...
def train(training_path, train_idx, test_idx):
    ...
    if config.in_channels == 3:
        selected_dataset = VesselsDataset_in3
    elif config.in_channels == 5:
        selected_dataset = VesselsDataset_in5
    else:
        raise ValueError('Unsupported number of input channels: {}'.format(config.in_channels))
...
def train_sets(sets):
    ...
    for i, set_ in enumerate(sets):
        ...
        if config.model in ['CMRRWNet', 'RRWNet', 'RRWNetAll', 'RRUNet']:
